...
window.Authorization_token = getCookie("authorization_token_fpgame_2");
if(search("test") == "true"){
...
function wechat_oauth(){
    ...
    .then((res)=>res.json())
    .then((res)=>{ 
      conssLog("authorization_token_fpgame_2",res.data);
        if(res.code == "success"){
...
window.if_p = function (){
    var scene = mugeda.scene;
  conssLog("if_p()",jlm[jle[0]], jlm[jle[1]]);
  if(jlm[jle[0]]==jlm[jle[1]]){// 相同
    bs_yes(jle[0],jle[1]);
    cgmp3.audio.play();
    // conssLog('if_p(m) 图案相同该加分',jle[0],jle[1]);
    ...
    if(jls_score>7){
      ...
      setTimeout(function(){
        ...
        clearInterval(setIntervaltTIME);
        jsmp3.audio.play();
    ...
    bs_err(jle[0],jle[1]);
    cwmp3.audio.play();
    // conssLog('if_p(m) 图案不相同',jle[0],jle[1]);
  }
};
// 执行透明图层盖着恢复
...
function ys(){
  ...
  */
  y_s.sort(randomsort);
  var ys23 = scene.getObjectByName("init");
  ys23.text = y_s.toString();
...
// 获取当日游戏可用信息
window.getNowDayUserGameInfo = function(){
    var scene = mugeda.scene;
  layer.open({
    type: 2
    ,shadeClose: false
  ...
  .then((res)=>{
    ...
    if(res.code == "success"){
      ...
      */
      ...
      var wxtx = scene.getObjectByName("头像");
      ...
      // layer.open({content:res.msg, btn: ['好的']});
      layer.open({content:res.msg, btn: ['好的']});
    }
    // getQrCode()
...
// isGoGame
window.isGoGame = function(){
  layer.open({
    type: 2
    ,shadeClose: false
    ,content: '等等返回数据...'
  });
  fetch(interfaceAddr +"isGoGame"+"?CampaignID="+CampaignID,{
    headers: {
      'Authorization': Authorization_token,
    },
    method: 'GET', // *GET, POST, PUT, DELETE, etc.
...
// 提交成绩
function putAchievement() {
    var scene = mugeda.scene;
  // kkey = scene.getObjectByName('s56').text;
  layer.open({
    type: 2
    ,shadeClose: false
    ,content: '正在提交成绩...'
  ...
  .then((res)=>{
    ...
    if(res.code == "success"){
      ...
      var mingci = scene.getObjectByName('mingci');
      var BFB = (res.data.MC/res.data.N) *100;
...
mugeda.defineCallback("dom",function(em){
  fetch("https://iuu.pub/v2/api/fpgame/" + "do?m="+em.name+"&v="+em.text+"&CampaignID="+CampaignID,{
    headers: {
      'Authorization': Authorization_token,
    },
...
window.getShareGameCount = function() {
  layer.open({
    type: 2
    ,shadeClose: false
    ,content: '正在查询服务器中...'
...
// goGame = () => {
    ...
    if(GameCount<1){
      ...
      sb_cw();
      scene.nextPage();
      window.setIntervaltTIME = setInterval('jsq_f();', 10);
    }
};
...
mugeda.addEventListener("renderready", function(){
  ...
  // 默认的盖子SRC
  window.gksrc = scene.getObjectByName("a0").src;
  //加载图片函数
  // load_img();
  //执行映射关系
  ys();
