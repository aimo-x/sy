...
const gacmotor_voice_heart_id = "aec30cd2-1a2d-5294-adef-36059154560c"
const gacmotor_voice_heart_id_test = "fa857b22-e92e-52ce-b428-8c2ed217a6ea"
...
next_form = () => {
  ...
  var job_code = scene.getObjectByName("工号").text
...
next_form_2 = ()=> {
  ...
  var mouthpiece_desc = scene.getObjectByName("代言人说明").text
  var models = scene.getObjectByName("车型").text
  if(mouthpiece.length<1){
    swal("请输入代言人姓名","", "error");
    return
  }
  if(mouthpiece_desc.length<1){
    swal("请输入代言人说明","", "error");
    return
  }
  ...
  scene.gotoAndPause(2, 8); // 跳转到相对于某页的某帧并暂停
}
...
create = () => {
  ...
  loading.src = "https://www.mugeda.com/c/user/data/5e367035cec58904ea6d1c30/5e5b0fbde49bc67b98259e44.gif"; // scene.getObjectByName("加载动画").src // "/c/user/data/5e367035cec58904ea6d1c30/5e5b0fbde49bc67b98259e44.gif"// 
  //loading.type = "range"
  swal({
    //text:"loading",
    content: loading,
  ...
  */
  ...
  var data = {
    ...
    mouthpiece_desc:mouthpiece_desc,
    models:models,
    slogan:slogan,
  ...
  fetch('https://'+domain+'/v2/api/gacmotor_voice_heart',{
    ...
    .then(function(res) {
        ...
        if(res.code == "success" ){
          // swal("提交成功","", "success");
          scene.gotoAndPause(3, 8); // 跳转到相对于某页的某帧并暂停
        }else{
