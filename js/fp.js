...
function wechat_oauth(){
    ...
    .then((res)=>res.json())
    .then((res)=>{ 
      console.log("authorization_token_fpgame_2",res.data);
        if(res.code == "success"){
...
//通关或者重玩重置函数
function sb_cw(){
  // window.setIntervaltTIME = setInterval('getTime();', 100);
...
window.getTime = function (){
  ...
  fetch(interfaceAddr +"getTime"+"?CampaignID="+CampaignID,{
    headers: {
      'Authorization': Authorization_token,
    },
    method: 'GET', // *GET, POST, PUT, DELETE, etc.
...
// goGame = () => {
    ...
    if(GameCount<1){
      ...
      window.setIntervaltTIME = setInterval('getTime();', 250);
      sb_cw();
      scene.nextPage();
    }
};
...
window.re_start=function(){
    // 设置重置属性
    // window.setIntervaltTIME = setInterval('getTime();', 10);
    // window.setIntervaltTIME = setInterval('getTime();', 10);
    gkname = gkArr[0];
    // 重新排列
    ys();
