...
function if_p(){
  if(jlm[jle[0]]==jlm[jle[1]]){//相同
    ...
    if(jls_score<6 && jls_score%3==0){
      // 未通关时只需
      if(jls_score<36){
        // gk
        var gk = scene.getObjectByName('gk');
        gk.text = jls_score/3 +  1 
        gkname = gkArr[jls_score/3]
        // gksys 
        var gksys = scene.getObjectByName("gksys").scene;
        gksys.gotoAndPause(jls_score/3)
